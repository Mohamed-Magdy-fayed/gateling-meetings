...
- **Plans and organizations** — every account owns a personal organization; the org's plan (`free` / `pro` / `business` / `unlimited`, see `src/features/billing/plans.ts`) caps participants, meeting length, upcoming scheduled meetings, breakouts, API access and — on Free — a monthly allowance of participant-minutes summed from the attendance log (600/month; the one cap that tracks the media bill). Free is 5 people / 40 minutes. Enforced at the join door, the meeting service and an Inngest duration enforcer — never only in the UI. `ADMIN_EMAILS` accounts are unlimited, and so is any org an admin puts on the comp-only `unlimited` plan (never sold; always a `manual` source).
- **Billing** — Paymob (Egypt) behind a provider-agnostic core: the buyer enters a billing name and phone, the server opens a `billing_checkouts` row bound to the org and a Paymob *intention* on a per-seat subscription plan, and the browser is sent to Paymob's hosted checkout (a top-level navigation; nothing is framed). Paymob tokenises the card and runs the recurring deductions itself. The HMAC-signed transaction callback lands in `billing_events` (idempotent per provider event id) and is applied by Inngest (`on-billing-webhook`): the initial payment grants the plan straight from the checkout row, recurring payments move the period forward or mark the org `past_due`, and the plan-level subscription webhook is only ever a hint to re-read the subscription from Paymob's API (`src/integrations/paymob/`). Status → access lives in `subscriptionAccess` (`src/features/billing/server/subscription-mapping.ts`): `active`/`trialing` grant, `past_due` gets a 7-day grace, `paused` and an effective `canceled` drop to free. Seat changes update the Paymob amount for the next cycle (no proration); cancel stops future charges at once and the org keeps its plan until `planExpiresAt`. Prices are EGP amounts in `src/features/billing/tiers.ts`. A plan set by hand in the admin panel is never overwritten by billing. Swapping providers means one adapter implementing `BillingProvider` (`src/features/billing/server/provider.ts`).
...
- **AI companion** — a chat in the header (sparkles icon) that starts, schedules and links meetings for the signed-in person in plain words ("schedule a meeting tomorrow at 12 PM with a@x.com and give me the link"). Built on TanStack AI with the Gemini adapter (`GEMINI_API_KEY`; hidden when unset) and kept cheap by design: a per-day message allowance by plan (Free 10, paid 100, unlimited/admin 300), a 10-per-minute burst limit, 1 000-character messages, only the last 8 turns sent, 400 output tokens, 3 tool rounds, an org-wide monthly cap (`COMPANION_MONTHLY_CAP`) and a meetings-only system prompt. Tools call the same meeting service the dashboard uses, as the signed-in person.
...
- **Integration API** — other Gateling systems create meetings and send their signed-in users in through a signed `/sso/join` link, and get signed webhooks back. See [docs/integration.md](docs/integration.md), [docs/webhooks.md](docs/webhooks.md) and [docs/mcp.md](docs/mcp.md) (the same key drives an MCP server at `/api/mcp`); Business-plan organizations mint their own keys at `/settings/integrations` (platform admins can mint uncapped platform keys).
...
6. **Paymob** — a merchant account (commercial register + tax card + national ID + bank account; ask Paymob about a sole-proprietorship route), an online-card integration in EGP, and the four dashboard keys (`PAYMOB_API_KEY`, `PAYMOB_SECRET_KEY`, `PAYMOB_PUBLIC_KEY`, `PAYMOB_HMAC_SECRET`). Pick a random `PAYMOB_SUBSCRIPTION_WEBHOOK_TOKEN`, set `BASE_URL` to the public https URL of the deployment the plans are for (not the local `.env` value — the seeder refuses http), then `npx tsx scripts/seed-paymob-plans.ts` creates the four subscription plans (per seat, monthly = 30 days, yearly = 360 — Paymob's `frequency` is a fixed choice list, `use_transaction_amount`) with the subscription webhook already registered and prints the `PAYMOB_PLAN_ID_*` lines. In the dashboard set the integration's *transaction processed callback* to `https://<your-app>/api/billing/webhook/paymob/transaction`. Set `BILLING_PROVIDER=paymob` and `PAYMOB_MODE=live` (the build refuses anything else in production unless `PAYMOB_ALLOW_TEST_IN_PRODUCTION=true` is set on purpose — the way to run the live site against Paymob's test environment while the merchant account is still being verified; remove it when switching to live keys). Test first with the test keys and test integration id (`PAYMOB_MODE=test`; Paymob's test card is 5123456789012346, 01/39, CVV 123) — callbacks need a public https URL, and on a Vercel-protected preview the callback URLs carry the automation bypass secret automatically. Amounts live in `src/features/billing/tiers.ts` (EGP, per seat); change them there, not in Paymob.
