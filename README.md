...
- **Billing** — Paddle (merchant of record): overlay checkout from `/settings/billing` or `/pricing`, per-seat subscriptions, customer portal, cancellation at period end. Webhooks land in `billing_events` (idempotent by Paddle event id) and are applied by Inngest: the Paddle entity is mirrored as-is into `paddle_customers` / `paddle_subscriptions` (upserts keyed on Paddle ids, never regressed by an older event), then the org's plan is derived from the subscription status (`subscriptionAccess` in `src/features/billing/server/subscription-mapping.ts`: `active`/`trialing` grant, a scheduled cancel does not revoke, `past_due` gets a 7-day grace, `paused` and an effective `canceled` drop to free). `/settings/billing/portal` mints a Paddle customer portal session server-side and redirects to it. A plan set by hand in the admin panel is never overwritten by billing.
